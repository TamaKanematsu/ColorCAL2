...
function createCrossScreen(winPtr, stimulus_rect, monitor_size)
    ...
    Screen('FillRect', winPtr, backRGB);
    Screen('DrawLine', winPtr ,crossRGB, 0, line_center(1), monitor_size.width, line_center(1)); % vertival line
    Screen('DrawLine', winPtr ,crossRGB, line_center(2), 0, line_center(2), monitor_size.height); % horizontal line
    DrawFormattedText(winPtr, '1. Remove the cover 2. Place ColorCAL2 in front of the cross\n\nPress any key to start the measuring', 'center', 'center', strRGB);
